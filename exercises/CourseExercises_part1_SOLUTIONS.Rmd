...
for (i in seq(1, nchar(my_DNA), by=3)) {
  codon <- substr(my_DNA, i, i+2);
  my_prot <- paste(my_prot, codons[[codon]], sep = "")
}
print(my_prot)
```
...
for (i in 1:nchar(my_DNA)) {
    current_nuc <- substr(my_DNA, i, i)
    current_nuc_weight <- nuc_weights$weights[nuc_weights$nucs == current_nuc]
    DNA_weight <- DNA_weight + current_nuc_weight
}
DNA_weight
```

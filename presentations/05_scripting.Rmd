...
So far you have only seen R code used in the console, in code chunks of an RMarkdown document, or maybe in an R script in the form of a scratchpad.  
The code you have seen consisted of (series of) R statements with one or more function calls.  
No conditional code, no repeated operations and no extraction of blocks of code into something reusable, a custom function.

This presentation is about those aspects.
...
- These concepts are used for flow control  
    - Conditional execution 
    - Repeated execution
    - Repeated conditional execution
...
- Functions are named blocks of code with a **_single well-defined purpose_** which make them reusable.
- They ususally have some data as input: its **parameters**
- They usually have some **return** value
- You have already used many **_predefined_** ones: `str`, `max`, `read.table` etc. If you type the name of a function without parenthesis you get its definition
...
A function
- usually, but not always, has a name.
- has a **_parameter list_** (sometimes of size zero) between parentheses
- has a **_method body_**
- may have a **_return value_**
...
Note that `return()` is a **_method call_** which is very unlike other programmi ng languages.

## The return statement is optional
T

Since the last statement of a method body is its **_implicit return value_**, the previous example is equivalent to this:
...
- the parameters without default value are compulsory
- the unnamed arguments should come first
- the unnamed arguments should be passed in the order in which they are declared
...
- An R script is a text file, usually with the extension `.R` that contains R code.
- When it is loaded, it is immediately evaluated.
...
- Just call `source(path/to/your/script)`
- Then use defined functions and variables
...
- To run R code from the commandline

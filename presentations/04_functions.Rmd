...
This is especially handy with intermediate results in long analysis workflows.
...
```{r}
animals = c("Chimpanzee", "Chimp", "chimpanzee", "Pan troglodytes")
animals == "Chimpanzee" | animals == "Chimp" | animals == "chimpanzee"
```

The solution here is not using literals, but describe **_patterns_**.  
...
In programming  we use **_regular expressions_** or **_RegEx_** to describe such patterns:
...
- **character classes** - A listing of possibilities for a single position. Between brackets: `[adgk]` means 'a' or 'd' or 'g' or 'k'. Use a hyphen to create a series: `[3-9]` means digits 3 through 9 and `[a-zA-Z]` means all alphabet characters. `[^adgk]` means anything _but_ a, d, g or k. A special case is `.`: any character.
- **alternatives** - Are defined by the pipe symbol `|` 
- **quantifiers** - How many times the preceding block should occur. See next slide
...
How would you define a regular expression that is equally precise?

Ambiguity codes can be found
...
Because the pattern is there, embedded in a longer string.  
...
Create a RegEx that will determine whether an element contains a Dutch zip code.
...
This is outside the scope of this first acquaintance. The `stringr` package from the tidyverse is much better at this than the base R functions. And the tidyverse is in the next course...
